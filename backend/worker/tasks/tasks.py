...
def get_result(job_id: str, keywords: list = None):
    ...
    with DatabaseSessionManager() as db_session:
        ...
        keywords_filtered, english_words = filter_text(json.loads(job.whisper_result.text), all_key_words, "/opt/app")

        model_llm, tokenizer_llm, device = get_model_and_tokenizer()
        final_keywords = get_key_stage2_llm(keywords_filtered, json.loads(job.whisper_result.text), model_llm, tokenizer_llm, device, "f")
        final_keywords = [w.capitalize() for w in final_keywords]

        result = parse_file(json.loads(job.whisper_result.text)['text'], description)
